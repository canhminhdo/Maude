...
Parser::extractFirstSubparse(int nodeNr) {
    ...
    for (int pos = endPos; pos >= 0; --pos) {
        ...
        if (pos == endPos) {
            ...
            //
            IntBoolPair retPair = findReturnOrDeterministicReductionPath(firstReturns[tokenNr], ruleNr, startTokenNr,
                                                                         tokenNr);
            returnIndex = retPair.first;
            Assert(returnIndex != NONE, "missing first return for rule " << ruleNr <<
                                                                         "  pos = " << pos << "  startTokenNr = "
                                                                         << startTokenNr);
            //
            //	Check for ambiguity if we don't know we're ambiguous
            //
            if (!ambiguous &&
                findReturnOrDeterministicReductionPath(returns[returnIndex].nextReturn, ruleNr, startTokenNr,
                                                       tokenNr).first != NONE)
                ambiguous = true;
            //
...
Parser::extractNextSubparse(int nodeNr) {
    ...
    for (; son != NONE; ++pos) {
        ...
        if (pos == endPos) {
            ...
            //
            IntBoolPair retPair = findReturnOrDeterministicReductionPath(nextReturnToCheck, p.ruleNr, p.startTokenNr,
                                                                         endTokenNr);  // pos is implicit
            returnIndex = retPair.first;
            if (returnIndex == NONE) {
                //
