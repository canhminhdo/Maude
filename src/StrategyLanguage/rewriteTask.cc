          ...
          pending(pending)  // copy pending stack
{
    ...
    newContext = baseContext->makeSubcontext(rcf->makeLhsInstance(*substitutionSoFar),
                                             RewritingContext::CONDITION_EVAL);
    newContext->reduce();
    //
...
RewriteTask::executionSucceeded(int resultIndex, StrategicProcess *insertionPoint) {
    ...
    RewritingContext *matchContext = rewriteState->getContext()->makeSubcontext(result,
                                                                                RewritingContext::CONDITION_EVAL);
    //
