...
ephemeralGC() {
    ...
    for (int i = 0; i < nrArgs; i++) {
        Node *t = g.args[i];
        if (t >= ephemeralStart && t < ephemeralEnd) EVAC(t, g.args[i], cn, ce)
    }
    ...
    for (Link *s = g.safePtr; s != 0; s = s[0].l) {
        ...
        for (int i = 2; i < nrSlots + 2; i++) {
            Node *t = s[i].n;
            if (t != 0 && t >= ephemeralStart && t < ephemeralEnd) EVAC(t, s[i].n, cn, ce)
        }
    }
    ...
    for (int i = copyStartChunk; i <= curChunk; i++) {
        ...
        for (;;) {
            ...
            for (int j = 0; j < nrArgs; j++) {
                Node *t = n->args[j];
                if (t >= ephemeralStart && t < ephemeralEnd) EVAC(t, n->args[j], cn, ce)
            }
        }
    }
...
static itimerval init = {{1000000, 0},
                         {1000000, 0}};
