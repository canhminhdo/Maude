...
Rope::operator+(const Rope &other) const {
    ...
    size_type needed = (n->height) < RELAXATION ? 1 : fiboTable[n->height -
                                                                RELAXATION];  // relaxed requirement for balance since we won't be returning strictly balanced ropes
    if (n->length < needed) {
...
Rope::substr(size_type offset, size_type length) const {
    ...
    incCount(left);
    incCount(right);
    return Rope(left).substr(offset, nrLeftChars) +
           Rope(right).substr(0, length - nrLeftChars); // substring spans both children
}
