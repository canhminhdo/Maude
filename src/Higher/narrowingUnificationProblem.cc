...
NarrowingUnificationProblem::findOrderSortedUnifiers() {
    ...
    DebugAdvisory("allocated " << nextBddVariable << " BDD variables" <<
                               " sc vars " << sortConstrainedVariables.size() << " freeVariables "
                               << freeVariables.size());
    //
    ...
    Bdd unifier = bddtrue;
    {
        for (int fv : sortConstrainedVariables) {
            if (fv >= substitutionSize) {
                ...
                DebugAdvisory(
                        "NarrowingUnificationProblem::findOrderSortedUnifiers() : Adding constraint for free, non-original variable; nodes in new bdd = "
                                <<
