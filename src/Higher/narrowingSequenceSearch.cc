...
NarrowingSequenceSearch::NarrowingSequenceSearch(RewritingContext *initial,
    ...
    DagNode *newDag = initial->root()->instantiate(s,
                                                   false);  // not safe if we haven't determined ground terms in context->root()
    if (newDag == 0)
        newDag = initial->root();
    ...
    //
    NarrowingSearchState *initialState = new NarrowingSearchState(redContext, freshVariableGenerator, true, UNDEFINED,
                                                                  narrowingFlags);
...
NarrowingSequenceSearch::findNextMatch() {
    ...
    for (;;) {
        ...
        matchState = new MatchSearchState(
                initial->makeSubcontext(getStateDag()),  // CHECK: could we just use existing context?
                goal,
...
NarrowingSequenceSearch::findNextNormalForm() {
    ...
    while (currentState->findNextNarrowing()) {
        ...
        currentState = new NarrowingSearchState(newContext, freshVariableGenerator, !(currentState->isOdd()), UNDEFINED,
                                                narrowingFlags);
...
NarrowingSequenceSearch::findNextInterestingState() {
    ...
    while (currentIndex >= 0) {
        ...
        if ((maxDepth == NONE || currentIndex < maxDepth) && currentState->findNextNarrowing()) {
            ...
            seenSet.insert(newContext->root());
            stateStack.append(
                    new NarrowingSearchState(newContext, freshVariableGenerator, !(currentState->isOdd()), UNDEFINED,
                                             narrowingFlags));
            return true;
        }
        //
