...
SyntacticView::showView(ostream &s) {
    ...
    FOR_EACH_CONST(i, OpTermMap, opTermMap)s << "  op " << i->second.first << " to term " << i->second.second << " .\n";
    const StratExprMap &stratExprMap = getStratExprMap();
    FOR_EACH_CONST(j, StratExprMap, stratExprMap)s << "  strat " << j->second.call << " to expr " << j->second.value
                                                   << " .\n";
    s << "endv\n";
    //
