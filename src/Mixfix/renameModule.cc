...
ImportModule::donateOps2(ImportModule *copy, Renaming *renaming) {
    ...
    for (int i = 0; i < nrUserSymbols; i++) {
        ...
        if (nrUserDeclarations > nrImportedDeclarations) {
            ...
            int index = (renaming == 0) ? NONE : renaming->renameOp(
                    symbol);  // index of renaming that applies to symbol
...
ImportModule::localStatementsComplete() {
    ...
    {  // do we need this? CHECK
        FOR_EACH_CONST(i, Vector<ImportModule *>, parameterTheories)labels.insert((*i)->labels.begin(),
                                                                                  (*i)->labels.end());
    }
    //
    ...
    //
    FOR_EACH_CONST(i, Vector<ImportModule *>, importedModules)labels.insert((*i)->labels.begin(),
                                                                            (*i)->labels.end());  // what about transitive imports? CHECK
    ...
    if (canonicalRenaming == 0) {
        ...
        //	from baseModule.
        //
        FOR_EACH_CONST(i, set<int>, baseModule->labels) labels.insert(canonicalRenaming->renameLabel(*i));
    }
}
