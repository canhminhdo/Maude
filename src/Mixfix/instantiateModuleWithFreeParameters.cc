...
ImportModule::makeInstantiation(int moduleName,
    ...
    if (!handleInstantiationByParameter(copy, canonical, parameterMap, positionsInstantiatedParameter, arguments,
                                        moduleCache)) {
        //
    ...
    //
    if (!handleInstantiationByModuleView(copy, canonical, parameterMap, positionsInstantiatedParameter, arguments,
                                         moduleCache)) {
        //
        //	Likely a parameter theory sort clashing with a funky user sort name - pack up and go home.
        //
    ...
    for (int i = 0; i < nrParameters; ++i) {
        if (Parameter *p = dynamic_cast<Parameter *>(arguments[i])) {
            ...
            if (copy->findParameterIndex(parameterName) == NONE) {
                ...
                if (ImportModule *parameterTheory = moduleCache->makeParameterCopy(parameterName,
                                                                                   baseParameterTheory)) {
                    DebugAdvisory("For parameter " << Token::name(parameterNames[i]) << " bound to parameter " <<
                                                   parameterToken
                                                   << " from an enclosing object we have baseParameterTheory " <<
                                                   baseParameterTheory << " and make a new parameter theory copy "
                                                   << parameterTheory);
                    copy->addParameter(parameterToken, parameterTheory);
