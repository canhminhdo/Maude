...
MixfixParser::makeStrategy(int node) {
    ...
    switch (a.action) {
        ...
        case MAKE_BRANCH: {
            BranchStrategy::Action successAction = static_cast<BranchStrategy::Action>(actions[parser.getProductionNumber(
                    node)].data);
            BranchStrategy::Action failureAction = static_cast<BranchStrategy::Action>(actions[parser.getProductionNumber(
                    node)].data2);
            int child = 0;
        ...
        case MAKE_TEST: {
            ...
            if (parser.getNumberOfChildren(node) > 1)  // such that clause
                makeCondition(parser.getChild(node, 2), condition);
            s = new TestStrategy(makeTerm(parser.getChild(node, 0)), actions[parser.getProductionNumber(node)].data,
                                 condition);
            break;
        }
        case MAKE_REW: {
        ...
        case MAKE_CEQ: {
            ...
            if (flags.getFlag(VARIANT)) {
                if (condition.empty())
                    ...
                    IssueWarning(
                            LineNumber(lineNumber) << ": variant attribute not allowed for conditional equations.");
            }
