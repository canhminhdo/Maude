        ...
        if (symbolType.hasAttachments()) {
            {
                ...
                for (int i = 0; i < nrIdHooks; i++) {
                    ...
                    if (!(symbol->attachData(domainAndRange, purpose, data)));  // HACK
                }
            }
            {
                ...
                for (int i = 0; i < nrOpHooks; i++) {
                    ...
                    if (!(symbol->attachSymbol(purpose, p.opHooks[i].symbol)));  // HACK
                }
            }
            {
                ...
                for (int i = 0; i < nrTermHooks; i++) {
                    ...
                    if (!(symbol->attachTerm(purpose, copy)));  // HACK
                }
            }
        }
...
MixfixModule::getSMT_Info() {
    if (smtInfo.getConjunctionOperator() ==
        0)  // HACK - we could have a problem if an SMT signature didn't have a conjunction operator
    {
        const Vector<Symbol *> &symbols = getSymbols();
        ...
        for (int i = 0; i < nrSymbols; ++i) {
            ...
            if (basicType == SymbolType::SMT_SYMBOL || basicType == SymbolType::SMT_NUMBER_SYMBOL) {
                ...
                FOR_EACH_CONST(j, Vector<OpDeclaration>, opDecs) {
                    ...
                    FOR_EACH_CONST(k, Vector<Sort *>, domainAndRange) smtSortIndices.insert(
                                (*k)->getIndexWithinModule());
                }
            }
        }
    }
