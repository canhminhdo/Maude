...
MixfixModule::printSort(Vector<int> &buffer, const Sort *sort, int printFlags) {
    ...
    if (Token::auxProperty(name) == Token::AUX_STRUCTURED_SORT &&
        ...
        FOR_EACH_CONST(i, Vector<int>, parts) buffer.append(*i);
    } else
        buffer.append(name);
}
    ...
    if (TrivialStrategy *t = dynamic_cast<TrivialStrategy *>(expr))
        ...
        switch (t->getDepth()) {
            case -1:
                ...
                break;
            case 0:
                buffer.append(xmatch);
                break;
            default:
                buffer.append(amatch);
        }
        ...
        switch (s->getDepth()) {
            case -1:
                buffer.append(matchrew);
                break;
            case 0:
                buffer.append(xmatchrew);
                break;
            default:
                buffer.append(amatchrew);
        }
