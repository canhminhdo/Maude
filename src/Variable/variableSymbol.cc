...
VariableSymbol::computeGeneralizedSort(const SortBdds &sortBdds,
    ...
    int firstVariable = realToBdd[safeCastNonNull<VariableDagNode *>(
            subject)->getIndex()];  // first BDD variable for this variable
    int nrVariables = sortBdds.getNrVariables(
            rangeComponent()->getIndexWithinModule());  // number of BDD variables depends on sort
...
VariableSymbol::computeGeneralizedSort2(const SortBdds &sortBdds,
    ...
    sortBdds.appendVariableVector(
            realToBdd[safeCastNonNull<VariableDagNode *>(subject)->getIndex()],  // first BDD variable
