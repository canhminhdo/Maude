...
MetaLevelOpSymbol::legacyMetaUnify2(FreeDagNode *subject, RewritingContext &context, bool disjoint) {
    DebugAdvisory(
            Tty(Tty::CYAN) << "meta unfication call: " << Tty(Tty::GREEN) << (DagNode *) subject << Tty(Tty::RESET));
    //
        ...
        if (metaLevel->isNat(metaVarIndex) &&
            ...
            for (; lastSolutionNr < solutionNr; ++lastSolutionNr) {
                if (!(unification->findNextUnifier())) {
                    ...
                    delete unification;
                    result = disjoint ? metaLevel->upNoUnifierTriple(incomplete) : metaLevel->upNoUnifierPair(
                            incomplete);
                    goto fail;
                }
            }
