        ...
        if (downQid(f->getArgument(0), name) &&
            ...
            if (MixfixModule::canHaveAsParameter(m->getModuleType(), theory->getModuleType())) {
                ...
                IssueAdvisory(LineNumber(FileTable::META_LEVEL_CREATED) <<
                                                                        ": parameterization of " <<
                                                                        QUOTE(MixfixModule::moduleTypeString(
                                                                                m->getModuleType())) <<
                                                                        " " << m << " by " <<
                                                                        QUOTE(MixfixModule::moduleTypeString(
                                                                                theory->getModuleType())) <<
                    ...
                    if (ai.flags.getFlag(VARIANT)) {
                        if (condition.empty())
                            eq->setVariant();
                                ...
                                "variant attribute not allowed for conditional equation in meta-module " << QUOTE(m)
                                                                                                         << '.');
                    }
                    m->insertEquation(eq);
                    if (ai.metadata != NONE)
                    ...
                    if (ai.flags.getFlag(NARROWING)) {
                        if (condition.empty())
                            rl->setNarrowing();
                                ...
                                "narrowing attribute not allowed for conditional rule in meta-module " << QUOTE(m)
                                                                                                       << '.');
                    }
                    m->insertRule(rl);
                    if (ai.metadata != NONE)
